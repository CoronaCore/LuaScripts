...
function PremiumSystem.ItemStoneCommand(event, player, msg, _, lang)
    ...
    if (msg == PremiumItemCommand) and PremiumActive then -- Use "#premium item" for create a premium item
        if  player:HasItem(PremiumItemEntry) then
            player:SendBroadcastMessage(string.format("%s|CFFFE8A0E %s your already have the %s|r", PremiumSystem.Settings.SystemName, PlayerName, GetItemLink(PremiumItemEntry)))
            return false;
        else
            player:AddItem(PremiumItemEntry, 1)
            player:SendBroadcastMessage(string.format("%s|CFFFE8A0E Create|r %s", PremiumSystem.Settings.SystemName, GetItemLink(PremiumItemEntry)))
            return false;
        end
    end
end
